...
from leap.utils import get_data_path, convert_time_to_year_month
...
class Simulation:
    ...
    def get_num_new_agents(
        ...
        if year == min_year:
            num_new_agents = int(math.ceil(num_new_born / np.sum(df[df["age"] == 0]["prop"])))
    ...
    def get_new_agents(self, year: int) -> pd.DataFrame:
        ...
        num_new_agents = self.get_num_new_agents(year, self.min_year, num_new_born, num_immigrants)
            ...
            new_agents_df = pd.DataFrame(
                {
                    ...
                    "sex": list(
                        np.random.binomial(1, np.array(initial_population_df["prop_male"]))
                    ),
    ...
    def generate_initial_asthma(self, agent: Agent):
        ...
        if agent.has_asthma:
            ...
            agent.exacerbation_history.num_current_year = (
                self.exacerbation.compute_num_exacerbations(agent=agent, initial=True)
            )
    ...
    def reassess_asthma_diagnosis(self, agent: Agent, outcome_matrix: OutcomeMatrix):
        ...
        if agent.has_asthma:
            ...
            agent.exacerbation_severity_history.prev_year = (
                agent.exacerbation_severity_history.current_year
            )
    ...
    def update_asthma_effects(self, agent: Agent, outcome_matrix: OutcomeMatrix):
        ...
        **Mutates**
        ...
        agent.control_levels = self.control.compute_control_levels(agent.sex, agent.age)
        ...
        if agent.exacerbation_history.num_current_year != 0:
            agent.exacerbation_severity_history.current_year = (
                self.exacerbation_severity.compute_distribution(
                    agent.exacerbation_history.num_current_year, agent.total_hosp > 0, agent.age
                )
            )
            agent.total_hosp += agent.exacerbation_severity_history.current_year[3]
    ...
    def run(self, seed=None, until_all_die: bool = False):
        """Run the simulation using a monthly time cycle.
        ...
        self.outcome_matrix = OutcomeMatrix(
            until_all_die, self.start_time, self.end_time, self.max_age
        )

        # Loop over each time unit in the simulation
        pbar_times = tqdm(
            self.times, desc="Time", bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt}"
        )
        for time in pbar_times:
            pbar_times.set_description(f"Time {time}")
    ...
    def _process_time(self, time):
        ...
        year, month = convert_time_to_year_month(time, self.min_year)
        new_agents_df = self.get_new_agents(year)

        logger.info(
            f"Time {time} of {self.end_time}."
        ...
        for i in pbar:
            self._process_agent(time, new_agents_df, i)

    def _process_agent(self, time, new_agents_df, i):
        ...
        logger.info(f"| -- Time: {agent.time}, " f"age: {agent.age}")
    ...
    def _process_agent_outcome_events(self, agent: Agent, month):
        ...
        while (
            ...
            and agent.time <= self.max_time_horizon * 12
        ):
            if not agent.has_asthma:
                ...
                self.reassess_asthma_diagnosis(agent, self.outcome_matrix)
                logger.info(
            ...
            if self.death.agent_dies(agent):
            ...
            elif self.emigration.compute_probability(agent.year, agent.age, str(agent.sex)):
                agent.alive = False
                ...
                if agent.age <= self.max_age and agent.time <= self.max_time_horizon * 12:
                    logger.info(
                        f"| -- Time: {agent.time}, age: {agent.age}"
                    )
