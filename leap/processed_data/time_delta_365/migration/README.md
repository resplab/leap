...
`age`: Integer age, in the range 1-100. Age 0 (newborns) is excluded because births are
  handled separately by the birth model.

`sex`: String, `F` = female, `M` = male.

`projection_scenario`: Population growth type, one of:
  `LG` (low growth), `HG` (high growth), `M1`-`M5` (medium growth variants),
  `FA` (fast aging), `SA` (slow aging).
