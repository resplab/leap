...
To run the data processing for the antibiotic data, with data points taken every year:

```sh
cd LEAP
python3 leap/data_generation/antibiotic_data.py --time-delta P1Y
```

This will update the following data files:

1. `leap/processed_data/{time_delta_tag}/config.json`
2. `leap/processed_data/{time_delta_tag}/antibiotic_predictions.csv`
...
The output of the data generation for the `AntibioticExposure` module is a `.csv` file:

`antibiotic_predictions.csv`
...
`n_abx_μ` | `float` | the predicted number of antibiotics prescribed to infants at the given timepoint
...
`projection_scenario` | `str` | ``past`` for historical data, or one of the projection scenario IDs for future data: <br><ul><li>`LG`: low-growth projection</li><li>`HG`: high-growth projection</li><li>`M1`: medium-growth 1 projection</li><li>`M2`: medium-growth 2 projection</li><li>`M3`: medium-growth 3 projection</li><li>`M4`: medium-growth 4 projection</li><li>`M5`: medium-growth 5 projection</li><li>`M6`: medium-growth 6 projection</li><li>`FA`: fast-aging projection</li><li>`SA`: slow-aging projection</li></ul>
...
`n_birth` | `float` | number of births in the given province during the given time interval
`projection_scenario` | `str` | ``past`` for historical data, or one of the projection scenario IDs for future data: <br><ul><li>`LG`: low-growth projection</li><li>`HG`: high-growth projection</li><li>`M1`: medium-growth 1 projection</li><li>`M2`: medium-growth 2 projection</li><li>`M3`: medium-growth 3 projection</li><li>`M4`: medium-growth 4 projection</li><li>`M5`: medium-growth 5 projection</li><li>`M6`: medium-growth 6 projection</li><li>`FA`: fast-aging projection</li><li>`SA`: slow-aging projection</li></ul>
...
1. `leap/processed_data/{time_delta_tag}/death/life_table_{province}.csv` (generated by `death_data.py`)
2. `leap/processed_data/{time_delta_tag}/birth/initial_population.csv` (generated by `birth_data.py`)

The `life_table.csv` contains the probability of death during that time interval for each age, sex,
province, and projection scenario.

The `initial_population.csv` contains the number of people in a given age, sex, province,
and projection scenario, along with the number of births for that timepoint. This data is the net
number of people, factoring in death, immigration, and emigration.
...
`life_table.csv`. Then we calculate the net change in people using the `n_age` column in the
...
This will update the file `leap/processed_data/{time_delta_tag}/asthma_occurrence_correction.csv`. This file
...
This will update the file `leap/processed_data/{time_delta_tag}/asthma_occurrence_correction.csv` as described
above, and will also update `leap/processed_data/{time_delta_tag}/occurrence_calibration_parameters.json`:
