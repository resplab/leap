...
def load_population_data(
    ...
    """Load the population data for the given province, starting timepoint, and projection scenario.
    ...
    Args:
        ...
        min_timepoint: The starting timepoint for the population data.
        max_timepoint: The maximum timepoint for the population data.
    ...
    # Filter the population data by province and starting timepoint
    df = df.loc[
        (df["province"] == province) & (df["timepoint"] >= min_timepoint)
    ]
    ...
    # Filter the population data by projection scenario and timepoint
    df = df.loc[
        (df["projection_scenario"] == "past") & (df["timepoint"] <= CENSUS_TIMEPOINT)
        | (
            (df["projection_scenario"] == projection_scenario)
            & (df["timepoint"] > CENSUS_TIMEPOINT)
        )
    ]
    ...
    df = df.loc[df["timepoint"] <= max_timepoint]
    df = df.loc[df["age"] >= min_age]
...
def exacerbation_calibrator(
    ...
    df_population = load_population_data(
        time_delta, province, min_timepoint, max_timepoint, projection_scenario, min_age, max_age
    )
